...
/** One leg of a batch. `value`/`data` default to zero/empty. */
export type Erc7579Call = {
  readonly to: Address;
...
export function encodeExecutionMode(callType: CallType, execType: ExecType): Hex {
  // bytes1 callType | bytes1 execType | bytes4 unused | bytes4 selector | bytes22 payload
  return `0x${callType.slice(2)}${execType.slice(2)}${'00'.repeat(30)}` as Hex;
}
...
function assertValid(call: Erc7579Call, index: number): void {
 ...
 */
export function encodeBatchExecutionCalldata(calls: readonly Erc7579Call[]): Hex {
 ...
 */
export function encodeSingleExecutionCalldata(call: Erc7579Call): Hex {
 ...
 */
export function encodeKernelExecute(calls: readonly Erc7579Call[]): Hex {
  if (calls.length === 0) {
 ...
 */
export function isSameCallData(a: Hex, b: Hex): boolean {
 ...
 */
export function assertCallDataMatches(calls: readonly Erc7579Call[], actual: Hex): void {
