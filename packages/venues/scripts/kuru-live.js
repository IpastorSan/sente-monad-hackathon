...
import { faucetClaimCall, KURU_FAUCET, KuruVenue } from '../src/kuru/index.ts';
...
async function waitForOpenOrders(done) {
  for (let i = 0; i < 30; i++) {
...
fixtures['placeLimit'] = { hash: limitOrder.txHash, logs: [] };
...
fixtures['cancel'] = { hash: cancelled.txHash, logs: [] };
...
show(
  '  order',
  await venue.cancel({ symbol: SYMBOL, orderId: limitOrder.id }).catch((e) => e.message),
);
...
fixtures['placeMarket'] = { hash: marketOrder.txHash, logs: [] };
show(
