...
import { createKuruApi } from './api.ts';
...
export class KuruVenue {
  ...
  async getMarkets() {
    ...
    return listed.flatMap((api) => {
      const market = this.#markets.find((m) => isAddressEqual(m.address, api.marketAddress));
  ...
  async getOpenOrders(symbol) {
    ...
    return orders.flatMap((api) => {
      const market = wanted.find((m) => isAddressEqual(m.address, api.marketAddress));
  ...
  #prepare(market, params, request, price, timeInForce) {
  ...
  async #place(prepared, type, price) {
  ...
  async #slippageBound(market, params, side, maxSlippage) {
  ...
  async #history(accountId, market, ref) {
  ...
  async #terminalOrder(market, accountId, ref) {
...
function rejectReduceOnly(request) {
